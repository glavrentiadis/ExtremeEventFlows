...
def cross_entropy_torch(
    ...
    *,
    ...
    for j, chain in enumerate(chains_gm):
        ...
        pdf_prob = density_gm_torch(gm, mag, dt, lam_prop, b_prop, ln_mean, ln_sd)
        pdf_samp = density_gm_torch(gm, mag, dt, lam_samp, b_samp, ln_mean, ln_sd)
        pdf_upd = density_gm_torch(gm, mag, dt, lam_upd, b_upd, ln_mean, ln_sd)
...
def iter_cross_entropy(
    ...
    *,
    ...
    # 1) Generate chains (non-differentiable)
    chains_gm: List[List[dict]] = []
    h_array: List[bool] = []
